...
namespace Domain.Shared_kernel
{
  ...
  public sealed class VoteTransaction
  {
    ...
    public static VoteTransaction ForVoteSpend(
        ...
        long amount,
        string spendId)
        => new VoteTransaction(leagueId, userId, -amount, "vote_spend", null, optionId, spendId);
    ...
    public static VoteTransaction ForRewardCredit(
        ...
        long amount,
        string rewardItemId)
        => new VoteTransaction(leagueId, userId, amount, "reward_redeem", rewardItemId, null, null);

    public static VoteTransaction ForAdjust(
        LeagueId leagueId,
        UserId userId,
        long delta)
        => new VoteTransaction(leagueId, userId, delta, "adjust", null, null, null);
    ...
    public static VoteTransaction ForPackPurchase(
        LeagueId leagueId, UserId userId, long amount, string packId)
        => new VoteTransaction(leagueId, userId, -amount, "pack_purchase", packId, null, null);

    /// <summary>Points locked when a fan places a bid (balance debited).</summary>
    public static VoteTransaction ForBidEscrow(
        LeagueId leagueId, UserId userId, long amount, string listingId)
        => new VoteTransaction(leagueId, userId, -amount, "bid_escrow", listingId, null, null);

    /// <summary>Escrowed points returned when a fan is outbid.</summary>
    public static VoteTransaction ForBidRelease(
        LeagueId leagueId, UserId userId, long amount, string listingId)
        => new VoteTransaction(leagueId, userId, amount, "bid_release", listingId, null, null);
    ...
    public static VoteTransaction ForAuctionSaleCredit(
        LeagueId leagueId, UserId userId, long amount, string listingId)
        => new VoteTransaction(leagueId, userId, amount, "auction_sale", listingId, null, null);
    ...
    public static VoteTransaction ForBuyNowDebit(
        LeagueId leagueId, UserId userId, long amount, string listingId)
        => new VoteTransaction(leagueId, userId, -amount, "buy_now", listingId, null, null);
    ...
    public static VoteTransaction ForH2HWager(
        LeagueId leagueId, UserId userId, long amount, string matchId)
        => new VoteTransaction(leagueId, userId, -amount, "h2h_wager", matchId, null, null);
    ...
    public static VoteTransaction ForH2HWin(
        LeagueId leagueId, UserId userId, long amount, string matchId)
        => new VoteTransaction(leagueId, userId, amount, "h2h_win", matchId, null, null);
  }
}
