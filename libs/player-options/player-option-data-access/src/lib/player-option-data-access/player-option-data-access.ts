...
export const PlayerOptionStore = signalStore(
  ...
  withMethods((store, http = inject(HttpClient)) => {
    ...
    return {
      ...
      loadPlayerOptions(
        userId = "user-123",
        filters?: Partial<PlayerOptionFilters>
      ): void {
        ...
        http
          .get<{ success: boolean; data: PlayerOption[]; message: string }>(
            `${API_BASE_URL}/PlayerOption/user/${userId}?${queryParams}`
          )
          .pipe(
              ...
              const mockOptions: PlayerOption[] = [
                {
                  ...
                  description:
                    "Should we trade our starting quarterback for draft picks?",
                  category: "trade",
                  status: "active",
                  createdAt: new Date(
                ...
                },
                {
                  ...
                  description:
                    "What position should we prioritize in the upcoming draft?",
                  category: "draft",
                  status: "active",
                  createdAt: new Date(
      ...
      loadPlayerOption(playerOptionId: string, userId?: string): void {
        ...
        http
          .get<{ success: boolean; data: PlayerOption; message: string }>(url)
          .pipe(
            map((response) => response.data),
            catchError((error) => {
              console.warn("API call failed, using mock data:", error);
      ...
      voteOnOption(voteRequest: VoteRequest): void {
        ...
        http
          .post<{ success: boolean; data: VoteResponse; message: string }>(
            `${API_BASE_URL}/PlayerOption/vote`,
            voteRequest
          )
          .pipe(
            map((response) => response.data),
      ...
      loadStats(userId?: string, organizationId?: string): void {
        ...
        http
          .get<{ success: boolean; data: PlayerOptionStats; message: string }>(
            `${API_BASE_URL}/PlayerOption/stats?${queryParams}`
          )
          .pipe(
            map((response) => response.data),
            catchError((error) => {
