...
export const RedeemStore = signalStore(
  ...
  withMethods((store, http = inject(HttpClient)) => {
    ...
    return {
      ...
      loadAvailableCodes(userId = "user-123"): void {
        patchState(store, { isLoading: true, error: null });

        http
          .get<{ success: boolean; data: RedeemCode[]; message: string }>(
            `${API_BASE_URL}/Code/available/${userId}`
          )
          .pipe(
      ...
      loadUserBalance(userId: string = "user-123"): void {
        http
          .get<{ success: boolean; data: UserBalance; message: string }>(
            `${API_BASE_URL}/Code/balance/${userId}`
          )
          .pipe(
            map((response) => response.data),
            catchError((error) => {
      ...
      redeemCode(request: RedeemRequest): void {
        ...
        http
          .post<{ success: boolean; data: RedeemResponse; message: string }>(
            `${API_BASE_URL}/Code/redeem-by-code`,
            request
          )
          .pipe(
            map((response) => response.data),
